...
const validate = (validations) => {
  return async (req, res, next) => {
    ...
    const validationStart = log('🚀 Starting validation middleware', {
    ...
    try {
      ...
      log('🔄 About to call next() asynchronously');
      
      // Use setImmediate to ensure we're in a fresh tick
      setImmediate(() => {
        log('✅ Inside setImmediate, calling next()');
        try {
          // Log the stack trace to see where we are
          const stackTrace = new Error('Debug stack trace').stack;
          log('📜 Stack trace before next()', { stackTrace });
          
          // Log the next function
          log('🔍 Next function:', {
            type: typeof next,
            name: next.name || 'anonymous',
            isAsync: next.constructor.name === 'AsyncFunction',
            nextFunction: next.toString().substring(0, 200) + '...'
          });
          
          // Call next
          const result = next();
          
          // Check if next() returned a promise
          if (result && typeof result.then === 'function') {
            log('🔮 next() returned a promise');
            result.catch(err => {
              log('❌ Error in async next() call', {
                error: err.message,
                stack: err.stack
              });
            });
          ...
          log('✅ next() called successfully');
        } catch (err) {
          log('❌ Error in validation middleware next() call', {
            error: err.message,
            stack: err.stack,
            errorType: err.constructor.name
          });
          // Try to call next with error
          try {
            next(err);
          } catch (nextErr) {
            console.error('❌❌ CRITICAL: Failed to call next with error', {
              originalError: err.message,
              nextError: nextErr.message,
              stack: nextErr.stack
            });
          }
        }
...
const messageValidationRules = {
  ...
  send: () => {
    ...
    return [
      body('content')
        .custom((value, { req }) => {
          console.log('[VALIDATION] Validating content field:', { 
            hasValue: !!value,
        ...
        .custom((value, { req }) => {
          console.log('[VALIDATION] Validating content length:', { 
        ...
        .custom((value, { req }) => {
          console.log('[VALIDATION] Content validation completed successfully', { 
            requestId: req.requestId || 'unknown',
            valueLength: value ? value.length : 0
          });
          return true;
  ...
  react: () => [
    body('reaction')
      ...
      .isIn(['like', 'love', 'laugh', 'wow', 'sad', 'angry', 'thumbsup', 'thumbsdown'])
